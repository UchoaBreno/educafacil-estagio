...
export default function Notas() {
  ...
  const [notas, setNotas] = useState<Record<number, Record<string, Nota>>>({});
  ...
  async function loadData() {
    ...
    if (alunosData) {
      ...
      const notasMap: Record<number, Record<string, Nota>> = {};
  ...
  function updateNota(alunoId: number, disc: string, field: keyof Nota, value: number | null) {
    setNotas(prev => ({
      ...prev,
      [alunoId]: {
        ...prev[alunoId],
  ...
  async function handleSave() {
    ...
    try {
      for (const alunoId of Object.keys(notas)) {
        const nota = notas[parseInt(alunoId)][disciplina];
        if (nota) {
          if (nota.id) {
