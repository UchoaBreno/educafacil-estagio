  ...
  const generatePDF = async () => {
    ...
    try {
      const { data: alunos } = await supabase
        .from('alunos')
        .select('id, nome, matricula, status, tipo_movimentacao, de_onde_veio, para_onde_vai, data_movimentacao')
        .eq('ano', parseInt(selectedAno))
        .or('status.eq.Transferido,tipo_movimentacao.eq.Transferência')
