  ...
  const generatePDF = async () => {
    ...
    try {
      const { data: turmas } = await supabase
        .from('turmas')
        .select('id, nome, serie, capacidade')
        .eq('ano', parseInt(selectedAno))
        .order('nome');

      const { data: alunos } = await supabase
        .from('alunos')
        .select('id, turma_id, status, tipo_movimentacao')
        .eq('ano', parseInt(selectedAno));
      ...
      const novasMatriculas = alunos?.filter(a => !a.tipo_movimentacao || a.tipo_movimentacao === 'Nova Matrícula').length || 0;
      const transferencias = alunos?.filter(a => a.tipo_movimentacao === 'Transferência').length || 0;
      const evasao = alunos?.filter(a => a.status === 'Transferido' || a.status === 'Desistente').length || 0;

      doc.setFontSize(14);
      doc.text('Resumo Geral:', 20, 55);
      
      doc.setFontSize(11);
      doc.text(`• Total de Matrículas: ${totalAlunos}`, 25, 65);
      ...
      if (turmas && turmas.length > 0) {
        const tableData = turmas.map(turma => {
          ...
          return [turma.nome, turma.serie, alunosTurma.toString(), capacidade.toString(), `${ocupacao}%`];
        });

        autoTable(doc, {
          startY: 100,
          head: [['Turma', 'Série', 'Alunos', 'Capacidade', 'Ocupação']],
          body: tableData,
          theme: 'grid',
